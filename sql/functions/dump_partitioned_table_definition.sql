CREATE FUNCTION @extschema@.dump_partitioned_table_definition(
  p_table_name TEXT,
  -- For native partitioned tables the template table needs to be
  -- created before the SQL generated by this function will work
  -- properly. If you haven't modified the template table at all
  -- then it's safe to pass TRUE here to have the generated SQL
  -- tell partman to generate a new template table. But for safety
  -- it's preferred to use pgdump to dump the template tables
  -- and restore them prior to using the generated SQL so that you
  -- can maintain any template overrides.
...
BEGIN
  ...
  WHERE pc.parent_table = p_table_name;
