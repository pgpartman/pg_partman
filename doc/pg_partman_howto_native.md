...
SELECT last_value FROM pg_catalog.pg_sequences WHERE schemaname||'.'||sequencename = pg_get_serial_sequence('public.original_table', 'col1');
...
You'll also notice that there is a child table missing in the set above (Dec 10, 2020). This is because we set the partition set to start 2 days ahead and we didn't have any data for that date in the original table. You can fix this one of two ways:
...
1. Run the "partman.partition_gap_fill()" function to fill any gaps immediately:
```
keith@keith=# SELECT * FROM partman.partition_gap_fill('public.original_table');
 partition_gap_fill 
--------------------
                  1
(1 row)
