 ...
 * `p_date_trunc_interval` - By default, pg_partman's time-based partitioning will truncate the child table starting values to line up on typical boundaries (daily, monthly, yearly, etc). If a custom time interval that does not fall on those typical time boundaries is desired this option may be required to ensure the child table have the expected boundaries. Especially if you also set `p_start_partition`. The valid values allowed for this parameter are the interval values accepted by the built in `date_trunc()` function (day, week, month, etc). For example, if you set a 9 week interval, by default pg_partman would truncate the tables by month (since the interval is greather than one month but less than 1 year) and unexpectdly start on the first of the month in some cases. Set this value to `week`, so that the child table start values are properly truncated on a weekly basis to line up with the 9 week interval. If you are using custom time interval, please experiment with this option so you get the expected set of child tables you desire. Otherwise use more typical time boundaries to simplify partition management.
