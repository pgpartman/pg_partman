...
SELECT create_partition('partman_test.time_taptest_table', 'col3', '1 week', p_epoch := 'seconds'
...
SELECT diag('!!! In separate psql terminal, please run the following (adjusting schema if needed): "CALL public.partition_data_proc(''partman_test.time_taptest_table'', p_wait := 0, p_source_table := ''partman_test.time_taptest_table_source'');".');
