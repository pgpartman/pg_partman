...
--      Ex  pg_prove -ovf -U partman_owner -d mydb test/test_nonsuperuser/test-id-nonsuperuser-part2.sql
...
SELECT is_empty('SELECT * FROM partman_test.id_taptest_table_default', 'Check that default table has no data');
...
SELECT undo_partition('partman_test.id_taptest_table', p_loop_count => 20, p_target_table => 'partman_test.undo_taptest', p_keep_table => false);
