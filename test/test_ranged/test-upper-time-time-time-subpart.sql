...
CREATE TABLE partman_test.time_taptest_table (col1 int primary key, col2 text, col3 tstzrange NOT NULL DEFAULT tstzrange(null, CURRENT_TIMESTAMP));
INSERT INTO partman_test.time_taptest_table (col1, col3) VALUES (generate_series(1,10), tstzrange(null, CURRENT_TIMESTAMP));

-- yearly
SELECT create_parent('partman_test.time_taptest_table', 'col3', 'partman', 'yearly', p_premake := 2, p_ranged_boundary := 'upper');
...
-- monthly
SELECT create_sub_parent('partman_test.time_taptest_table', 'col3', 'partman', 'monthly', p_premake := 2, p_ranged_boundary := 'upper');
...
-- daily
SELECT results_eq('SELECT create_sub_parent(''partman_test.time_taptest_table_p''||to_char(CURRENT_TIMESTAMP, ''YYYY''), ''col3'', ''partman'', ''daily'', p_premake := 2, p_ranged_boundary := ''upper'')',
...
SELECT results_eq('SELECT create_sub_parent(''partman_test.time_taptest_table_p''||to_char(CURRENT_TIMESTAMP+''1 year''::interval, ''YYYY''), ''col3'', ''partman'', ''daily'', p_premake := 2, p_ranged_boundary := ''upper'')',
...
SELECT results_eq('SELECT create_sub_parent(''partman_test.time_taptest_table_p''||to_char(CURRENT_TIMESTAMP+''2 years''::interval, ''YYYY''), ''col3'', ''partman'', ''daily'', p_premake := 2, p_ranged_boundary := ''upper'')',
...
INSERT INTO partman_test.time_taptest_table (col1, col2, col3) VALUES (generate_series(11,20), 'stuff', tstzrange(null, CURRENT_TIMESTAMP+'1 day'::interval));
...
INSERT INTO partman_test.time_taptest_table (col1, col2, col3) VALUES (generate_series(21,30), 'stuff', tstzrange(null, CURRENT_TIMESTAMP+'3 years'::interval));
